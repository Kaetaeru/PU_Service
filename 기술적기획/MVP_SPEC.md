...
>
> **이전 초안 문서.** 다음 항목은 현행 계약과 충돌하므로 사용하지 않는다.
> - §2.1 "입력 변경 시 요금 실시간 재계산 및 표시" — **Guest는 금액을 보지 않는다**
> - §3.1 `guest_paid` / `host_amount` / `payment_type` / `payment_status` — 지불은 서비스 바깥. `payment_confirmed` 하나만 쓴다
> - §3.1 `dispatch_status = waiting|assigned|notified` — 상태는 `00_CANONICAL_SYSTEM.md` §10으로 분리됐다
> - §4.1 "pax 범위를 벗어나면 가장 가까운 상위 구간 적용" — 8명 이상은 거부한다
> - §4.2 요금표 — `08_요금표.md`가 대체한다
> - §2.3 호스트가 차량번호·기사 연락처를 입력 — 대표기사가 직접 입력한다
> - 카톡 전송 — WhatsApp만 사용한다
>
> 유효한 부분: 화면 구성 의도, 경고 규칙, 메시지 템플릿 구조, 비기능 요구사항.
